...
class CredentialDetector(SyncDetector):
    ...
    CREDENTIAL_PATTERNS = [
        ...
        (
            r"(?:aws_access_key_id|aws_secret_access_key|aws_session_token)\s*[=:]\s*['\"]?[A-Za-z0-9/+=]{20,}",
            "aws_config",
            "high",
        ),
        ...
        (
            r"(?:tenant|subscription|client)[-_]?(?:id)?\s*[=:]\s*['\"]?[0-9a-f-]{36}",
            "azure_config",
            "medium",
        ),
        # GitHub
        ...
        (
            r"(?:secret[_-]?key|secretkey|secret[_-]?token)\s*[=:]\s*['\"]?[a-zA-Z0-9_\-]{20,}['\"]?",
            "secret_key",
            "high",
        ),
        (
            r"(?:access[_-]?token|auth[_-]?token|bearer)\s*[=:]\s*['\"]?[a-zA-Z0-9_\-]{20,}['\"]?",
            "access_token",
            "high",
        ),
        ...
        # Heroku
        (
            r"(?:heroku_api_key|heroku_api_token)\s*[=:]\s*['\"]?[a-f0-9-]{36}['\"]?",
            "heroku_api_key",
            "high",
        ),
        # Twilio
        ...
        # PayPal
        (
            r"(?:access_token$|client_id$|client_secret$)\s*[=:]\s*['\"]?[A-Za-z0-9]{40,}['\"]?",
            "paypal_credential",
            "high",
        ),
        # DigitalOcean
        ...
        # Cloudflare
        (
            r"(?:cloudflare_api_token|cf_api_token)\s*[=:]\s*['\"]?[a-zA-Z0-9_-]{40}['\"]?",
            "cloudflare_token",
            "critical",
        ),
        # NPM tokens
